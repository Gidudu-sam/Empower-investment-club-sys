...
$isActive     = $memberStatus !== 'inactive'; // toggle-button state: on (active/dormant) vs off (inactive)
        ...
        <div class="card text-center">
            <div class="card-body py-4 px-3">
                ...
                <div class="d-flex flex-column gap-2">
                    ...
                    <form method="POST" action="<?= APP_URL ?>/index.php?page=member-toggle"
                          onsubmit="return confirm('Change status to <?= $isActive ? 'Inactive' : 'Active' ?>?')">
                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken) ?>">
                        <input type="hidden" name="id" value="<?= $member['id'] ?>">
                        <button type="submit" class="btn btn-<?= $isActive ? 'warning' : 'success' ?> btn-sm w-100">
