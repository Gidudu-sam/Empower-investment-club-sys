...
class MemberModel extends Model
{
    ...
    public function syncDormantStatus(): void
    {
        try {
            ...
            $members = $this->db->query(
                "SELECT id, join_date FROM `members` WHERE status IN ('active','dormant')"
            )->fetchAll();
            if (!$members) return;
            ...
            $updStmt = $this->db->prepare(
                "UPDATE `members` SET `status` = ? WHERE `id` = ? AND `status` != ?"
            );
    ...
    /**
     * Manual admin toggle: Inactive (left/suspended) <-> Active.
     * A Dormant member counts as "on" here too, so toggling deactivates
     * them (stops dormancy monitoring); toggling an Inactive member
     * reactivates them and dormancy monitoring resumes.
     */
