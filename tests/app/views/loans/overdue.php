...
<div class="row g-3 mb-4">
    ...
    <div class="col-6 col-md-4"><div class="stat-card"><div class="stat-label">Total Outstanding</div><div class="stat-value" style="font-size:1.1rem;color:var(--rust);"><span style="font-size:.7rem;color:var(--slate-soft);">Shs</span> <?= number_format($totalOutstanding, 0) ?></div></div></div>
</div>
        ...
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="fw-semibold"><?= htmlspecialchars($memberLabel($l)) ?></td>
...
function generateReportText() {
    const loans = <?= json_encode(array_map(fn($l) => $memberLabel($l) . ' - ' . $l['loan_number'] . ' - Shs ' . number_format($l['outstanding'],0) . ' (' . $l['days_overdue'] . ' days)', $loans)) ?>;
    let text = 'OVERDUE LOANS - ' + <?= json_encode(date('d M Y')) ?> + '\n\n';
    loans.forEach((l, i) => { text += (i+1) + '. ' + l + '\n'; });
    text += '\nTotal Overdue: ' + <?= count($loans) ?>;
    text += '\nTotal Outstanding: Shs ' + (<?= (int)$totalOutstanding ?>).toLocaleString();
