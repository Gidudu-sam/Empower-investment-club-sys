<?php
$pageTitle = $pageTitle ?? 'Record Expense';
$title = $pageTitle;
$icon  = 'bi-receipt';
?>
    ...
    <?php if (empty($categories)): ?>
        <div class="alert alert-warning d-flex align-items-center gap-2">
            ...
            <div>
                No active expense categories with GL account mappings exist.
                <a href="<?= APP_URL ?>/index.php?page=expense-categories" class="alert-link">Set up categories first.</a>
            </div>
        </div>
    <?php endif; ?>

    <form method="POST" action="<?= APP_URL ?>/index.php?page=expense-store" id="expenseForm">
            ...
            <div class="col-12 col-xl-8">
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <i class="bi bi-tag me-2"></i>Expense Details
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="category_id" id="category_id" class="form-select" required
                                    ...
                                    <?php if (!empty($categoriesGrouped)): ?>
                                        <?php foreach ($categoriesGrouped as $group => $items): ?>
                                ...
                                <!-- Warning shown only if selected category has no GL — hidden by default -->
                                <div id="gl_warning" class="form-text text-warning" style="display:none;">
                                    <i class="bi bi-exclamation-triangle me-1"></i>
                                    This category has no GL account mapped. The expense can be saved as draft,
                                    but <strong>cannot be posted</strong> to the ledger until a GL account is set
                                    in <a href="<?= APP_URL ?>/index.php?page=expense-categories" target="_blank">Expense Categories</a>.
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-text">Date the expense was incurred, not necessarily today.</div>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" name="description" id="description"
                                       ...
                                       placeholder="e.g. Monthly office rent — August 2026">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label for="payee_name" class="form-label fw-semibold">Payee / Supplier</label>
                                <input type="text" name="payee_name" id="payee_name"
                                       class="form-control" placeholder="Who was paid?">
                            </div>
                            ...
                            <div class="col-md-6" id="externalReferenceField" style="display:none;">
                                ...
                                <input type="text" name="reference_number" id="reference_number"
                                       class="form-control" placeholder="Invoice or receipt number">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="<?= APP_URL ?>/index.php?page=expenses" class="btn btn-outline-secondary btn-lg">
                        Cancel
                    </a>
                </div>
            </div>
                ...
                <div class="card mb-3 border-primary">
                    ...
                    <div class="card-body">
                        <dl class="row mb-0 small">
                            ...
                            <dd class="col-7" id="sum_payee">—</dd>
                        </dl>
                    </div>
                </div>
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            ...
                            <tbody>
                                <tr>
                                    <td class="ps-3 text-muted" id="jnl_expense_acct">Expense account</td>
                                    <td class="text-end" id="jnl_dr">—</td>
                                    <td class="text-end pe-3 text-muted">—</td>
                                </tr>
                                <tr>
                                    <td class="ps-3 text-muted">1110 Cash at Hand</td>
                ...
                <div class="card border-0 bg-light">
                    <div class="card-body small text-muted">
                        <p class="mb-2">
                            ...
                            Expenses are saved as <strong>Draft</strong> first.
                        </p>
                        <p class="mb-2">
                        ...
                        <p class="mb-0">
                            ...
                            <strong>Dr</strong> the expense GL account,
                            <strong>Cr</strong> Cash at Hand.
                        </p>
                    </div>
                </div>
...
(function () {
    ...
    var datePick  = document.getElementById('expense_date');
    var amountIn  = document.getElementById('amount');
    var methodSel = document.getElementById('payment_method');
    var payeeIn   = document.getElementById('payee_name');
    ...
    function update() {
        ...
        // GL warning under the dropdown (only visible to user if no mapping)
        var glWarning = document.getElementById('gl_warning');
        if (glWarning) {
            glWarning.style.display = (catSel.value && !hasGl) ? '' : 'none';
        }

        // Summary sidebar — show clean category name, no GL codes
        ...
        // Journal preview — show GL code here (this is admin/treasurer view)
        document.getElementById('jnl_expense_acct').textContent = glCode
            ? glCode + ' — ' + glName
        ...
        // Payee
        document.getElementById('sum_payee').textContent = payeeIn.value.trim() || '—';
    }

    // Wire up all inputs
    [catSel, datePick, amountIn, methodSel, payeeIn, descIn].forEach(function (el) {
