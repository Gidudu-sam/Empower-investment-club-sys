...
class SavingsController extends Controller
{
    ...
    public function delete(): void
    {
        ...
        // Stage 9: deleting a savings row can orphan its journal entry
        // (SavingsModel::delete() now reverses it) -- admin-only, matching
        // the pattern already used for repayment deletion.
        if (Session::get('user_role') !== 'admin') {
            Session::flash('error', 'Only administrators can delete savings records.');
            $this->redirect(APP_URL . '/index.php?page=savings');
            return;
        }
        ...
        if ($deleted) {
            $this->model->log(
                $userId,
                'savings_deleted',
                "Deleted {$saving['receipt_number']} — Shs " . number_format($saving['amount'], 2) .
                " for {$saving['member_number']}"
            );
            Session::flash('success', "Savings record {$saving['receipt_number']} deleted.");
        } else {
            Session::flash('error', 'Could not delete the savings record.');
        }
    ...
    public function bulkDelete(): void
    {
        ...
        if (Session::get('user_role') !== 'admin') {
            Session::flash('error', 'Only administrators can bulk delete records.');
            $this->redirect(APP_URL . '/index.php?page=savings');
            return;
        }
        ...
        foreach ($ids as $id) {
            ...
            $saving = $this->model->findWithDetails($id);
            if ($saving) {
                // Stage 13-F3: was previously calling delete($id) with no
                // $userId at all (defaulting to 0), which -- for a posted
                // row -- would have attributed its journal reversal to
                // "user #0" instead of the real acting admin, undermining
                // AUTH-D-02's maker/checker attribution for bulk deletes.
                // Now also subject to SavingsModel::delete()'s posted-row
                // immutability guard: a posted row throws instead of being
                // silently skipped, so it's counted separately below rather
                // than treated the same as "not found".
                try {
                    if ($this->model->delete($id, $bulkUserId)) {
        ...
        if ($deleted > 0) {
            $this->model->log(
                ...
                ($skippedPosted > 0 ? " ({$skippedPosted} posted record" . ($skippedPosted > 1 ? 's' : '') . " skipped)" : '')
            );
            $msg = "Deleted {$deleted} savings record" . ($deleted > 1 ? 's' : '') .
                " (Shs " . number_format($totalAmount, 0) . ").";
            if ($skippedPosted > 0) {
                $msg .= " {$skippedPosted} record" . ($skippedPosted > 1 ? 's were' : ' was') .
                    " already posted to the ledger and could not be deleted — use the Controlled Corrections workflow to reverse " .
                    ($skippedPosted > 1 ? 'them' : 'it') . " instead.";
            }
            Session::flash('success', $msg);
        } elseif ($skippedPosted > 0) {
            Session::flash('error', "None of the selected records could be deleted: all {$skippedPosted} " .
                "selected record" . ($skippedPosted > 1 ? 's are' : ' is') . " already posted to the ledger. " .
                "Use the Controlled Corrections workflow to reverse " . ($skippedPosted > 1 ? 'them' : 'it') . " instead.");
        } else {
